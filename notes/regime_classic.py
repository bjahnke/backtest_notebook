...
def regime_floor_ceiling(df, _h, _l, _c, slo, shi, flr, clg, rg, rg_ch, stdev, threshold):
    ...
    # initialize floor_ceiling dataframe to store floor, ceiling, regime change values
    ...
    for i in range(loop_size):
        ...
        if (floor_found == False):
            ...
            if (floor_test >= threshold):
                ...
                ### Append lists
                # append data to floor_ceiling_data
...
def swings(df, bm_df, bm_col, ccy_col, start, end, lvl, d_vol, dist_pct, vlty_n, r_vol, retrace_pct, dgt, rel=False):
